...
a =  ((((R1 - R0 - S1 + S0)*n + (R0 - S0))*x + (S1 - S0)*n + S0) - (((T1 - T0 - P1 + P0)*n + (T0 - P0))* x + (P1 - P0)*n + P0))
area = (1 - exp(-1 * beta * a))
area1 = (1 - exp(beta * a))
print(area,area1)

# def solarea(area, x, n):
#     # if 1 - 0.135335283236613*exp(10.0*n + 1.0*x*(6.0*n - 6.0) - 1.0*x*(12.0*n - 7.0)) < 0:
#     #     return eval("0")
#     # else:
#     return eval("max(1 - 0.00673794699908547*exp(2.0*n*x + 7.0*n + 1.0*x*(3.0 - 6.0*n)),0)")
#
# def solarea1(area, x, n):
#     # if 1 - 7.38905609893065*exp(-10.0*n - 1.0*x*(6.0*n - 6.0) + 1.0*x*(12.0*n - 7.0)) < 0:
#     #     return eval("0")
#     # else:
#     return eval("max(1 - 148.413159102577*exp(-2.0*n*x - 7.0*n - 1.0*x*(3.0 - 6.0*n)),0)")


def solarea(area, x, n):
    return eval("max(1 - 0.135335283236613*exp(10.0*n + 1.0*x*(6.0*n - 6.0) - 1.0*x*(12.0*n - 7.0)),0)")

def solarea1(area, x, n):
    return eval("max(1 - 7.38905609893065*exp(-10.0*n - 1.0*x*(6.0*n - 6.0) + 1.0*x*(12.0*n - 7.0)),0)")
...
def pend(y, t, R0, S0, T0, P0, R1, S1, T1, P1, theta, episi, beta):
    x, n = y
    dydt = [x*(1 - x)*(solarea(area, x, n) - solarea1(area1, x, n)), episi*n*(1 - n)*(-1 + (1 + theta)*x)]
    return dydt
...
# plt.annotate(text='unstable limit cycle',xy=(0.8,0.02),xytext=(0.7,0.2), arrowprops=dict(facecolor='black', edgecolor='darkgrey',headlength=15, width=5, headwidth=18), font=font1, horizontalalignment='center', verticalalignment='center')
plt.annotate(text='stable limit cycle',xy=(0.5,0.95),xytext=(0.5,0.8), arrowprops=dict(facecolor='black', edgecolor='darkgrey',headlength=15, width=5, headwidth=18), font=font1, horizontalalignment='center', verticalalignment='center')

# plt.annotate(text='',xytext=(1 - c/(R0 * (c - VL)) - 0.003, 0.02),xy=(1 - c/(R0 * (c - VL)), 0), arrowprops=dict(facecolor='darkgrey', edgecolor='darkgrey',headlength=15,width=10))
