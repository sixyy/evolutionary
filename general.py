...
a, b = -oo, beta * (((R1 - R0 - S1 + S0)*n + (R0 - S0))*x + (S1 - S0)*n + S0) - (((T1 - T0 - P1 + P0)*n + (T0 - P0))* x + (P1 - P0)*n + P0)
area = sm.integrate(f(p), (p, a, b))
area1 = sm.integrate(f(p), (p, a, -b))
print(area)
print(area1)

def solarea(area, x, n):
    return eval("math.sqrt(2)*math.sqrt(math.pi)*math.erf(math.sqrt(2)*(-2.0*n*x - 7.0*n - x*(3.0 - 6.0*n) + 5.0)/2)/2 + math.sqrt(2)*math.sqrt(math.pi)/2")

def solarea1(area, x, n):
    return eval("-(math.sqrt(2)*math.sqrt(math.pi)*math.erf(math.sqrt(2)*(-2.0*n*x - 7.0*n - x*(3.0 - 6.0*n) + 5.0)/2)/2) + math.sqrt(2)*math.sqrt(math.pi)/2")

# def solarea(area, x, n):
#     return eval("math.sqrt(2)*math.sqrt(math.pi)*math.erf(math.sqrt(2)*(-10.0*n - x*(6.0*n - 6.0) + 1.0*x*(12.0*n - 7.0) + 2.0)/2)/2 + math.sqrt(2)*math.sqrt(math.pi)/2")
#
# def solarea1(area, x, n):
#     return eval("-(math.sqrt(2)*math.sqrt(math.pi)*math.erf(math.sqrt(2)*(-10.0*n - x*(6.0*n - 6.0) + 1.0*x*(12.0*n - 7.0) + 2.0)/2)/2) + math.sqrt(2)*math.sqrt(math.pi)/2")
...
def pend(y, t, R0, S0, T0, P0, R1, S1, T1, P1, theta, episi, beta):
    x, n = y
    dydt = [x*(1 - x)*(1/math.sqrt(2 * math.pi) * solarea(area, x, n) - 1/math.sqrt(2 * math.pi) * solarea1(area1, x, n)), episi*n*(1 - n)*(-1 + (1 + theta)*x)]
    return dydt
...
plt.annotate(text='unstable limit cycle',xy=(0.75,0.08),xytext=(0.7,0.25), arrowprops=dict(facecolor='black', edgecolor='darkgrey',headlength=15, width=5, headwidth=18), font=font1, horizontalalignment='center', verticalalignment='center')
# plt.annotate(text='stable limit cycle',xy=(0.5,0.88),xytext=(0.5,0.7), arrowprops=dict(facecolor='black', edgecolor='darkgrey',headlength=15, width=5, headwidth=18), font=font1, horizontalalignment='center', verticalalignment='center')
